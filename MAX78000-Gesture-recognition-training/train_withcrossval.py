...
import numpy as np
from datasets import customdataloader2
import matplotlib
...
def main():
    ...
    vloss = 10**6
    k_folds=5
    Xtrain, Ytrain, _, _=customdataloader2.GetXY_train_test()
    # train_dataset=CustomDataset
    train_dataset, _ = args.datasets_fn((os.path.expanduser(args.data), args), load_train=True, load_test=True)
    ...
    if args.deterministic:
        ...
        worker_init_fn = apputils.__deterministic_worker_init_fn
    for fold, (train_ids, test_ids) in enumerate(kfold.split(Xtrain, Ytrain)):
        confirmation = input(f"You want to begin with training fold number {fold} ? (yes/no): ")
        ...
        msglogger.info('\tFOLD %d',fold)
        msglogger.info('\t--------------------------------')
        train_subsampler = torch.utils.data.SubsetRandomSampler(train_ids)
        test_subsampler = torch.utils.data.SubsetRandomSampler(test_ids)
        train_loader = torch.utils.data.DataLoader(
                            train_dataset, 
                            batch_size=32, sampler=train_subsampler,num_workers=args.workers,pin_memory=pin_memory,worker_init_fn=worker_init_fn,collate_fn=args.collate_fn)
        val_loader = torch.utils.data.DataLoader(
                            train_dataset,
                            batch_size=32, sampler=test_subsampler,num_workers=args.workers,pin_memory=pin_memory,worker_init_fn=worker_init_fn,collate_fn=args.collate_fn)
        for epoch in range(start_epoch, ending_epoch):
            ...
            if compression_scheduler:
                compression_scheduler.on_epoch_end(epoch, kwargs=a)
        msglogger.info('\ttest accuracy for fold n %d',fold)
...
def evaluate_model(model, criterion, test_loader, loggers, activations_collectors, args,
    ...
    a=images[args.generate_sample]
    b=labels[args.generate_sample]
    print("aaaaaaaaaaa", b)
    print(labels)
